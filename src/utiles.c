...
REQUEST_QUEUE 	xdata commandQueue;
CALLBACK_QUEUE	xdata callbackQueue;
...
BOOL  SendQueueSysRuningParam(  uint8_t pressure, uint8_t temperature,   uint8_t battery  )
{
  ...
  if (commandQueue.requestCnt < MAX_CALLREQ_QUEUE)
  {
	...
	commandQueue.requestQueue[commandQueue.requestIn].wBuf[1]=temperature;
	commandQueue.requestQueue[commandQueue.requestIn].wBuf[2]=battery;
	commandQueue.requestIn++;
...
void PopCommandQueue(void)
{
  ...
  {
    commandQueue.requestOut = commandQueue.requestIn=0;
  }
...
void PopAckQueue(void)
{
  ...
  {
    callbackQueue.requestOut = callbackQueue.requestIn=0;
  }
